...
(defun account-trade-list (key secret symbol &optional (start-time nil start-time-supplied-p) (end-time nil end-time-supplied-p) (from-id nil from-id-supplied-p) (limit nil limit-supplied-p)(recvWindow 5000))
  (let* ((timestamp     (get-timestamp))
         (query-string1 (concatenate 'string "symbol=" symbol))
	 (start-time-qs (if start-time-supplied-p (concatenate 'string "&startTime=" (princ-to-string start-time)) ""))
	 (end-time-qs   (if end-time-supplied-p   (concatenate 'string "&endTime="   (princ-to-string end-time)) ""))
	 (from-id-qs    (if from-id-supplied-p    (concatenate 'string "&fromId="    (princ-to-string from-id)) ""))
	 (limit-qs      (if limit-supplied-p      (concatenate 'string "&limit="     (princ-to-string limit)) ""))
         (query-string3 (concatenate 'string "&recvWindow=" (princ-to-string recvWindow) "&timestamp=" (princ-to-string timestamp))) 
         (query-string  (concatenate 'string query-string1 start-time-qs end-time-qs from-id-qs limit-qs query-string3))
	 (signature (hex (hmac_sha256 secret query-string)))
	 (parameter (concatenate 'string "?" query-string "&signature=" signature))
